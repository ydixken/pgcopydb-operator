...
- E2e runs in **two places**. Locally, `task e2e` targets the dev cluster through the developer's own kubeconfig context and asks first. In CI, `release.yml` runs the same specs against a release candidate at `E2E_SCALE=0.25`, on a runner scale set that may reach the dev cluster, inside namespaces it neither creates nor deletes. That candidate is cut weekly, so the run's few hours buy a throughput reading the old 0.1 tier was too small to give.
