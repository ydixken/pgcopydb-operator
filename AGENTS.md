...
- E2e runs in **two places**. Locally, `task e2e` targets the dev cluster through the developer's own kubeconfig context and asks first. In CI, `release.yml` runs the same specs against a release candidate at `E2E_SCALE=0.1`, on a runner scale set that may reach the dev cluster, inside namespaces it neither creates nor deletes. The small tier keeps the gate short enough to sit through, and it still covers every scenario, but its fixtures are too small for the copy rate to mean anything: take throughput readings from a local `task e2e` instead.
