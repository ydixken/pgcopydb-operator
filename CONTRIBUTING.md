...
| `E2E_EXTRA_TABLES`            | unset   | Adds this many extra tables on top of the v2 fixture, with sizes drawn from a normal distribution and normalised to `E2E_EXTRA_SIZE_GB`. The v2 fixture is deliberately lopsided (one table holds 73% of the bytes); this gives it a production shape. Must be set with `E2E_EXTRA_SIZE_GB`. |
