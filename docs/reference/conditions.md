...
| `CutoverCompleted` | normal-true | The drain is proven: origin progress exactly at the cutover LSN, or a clean data compare for any distance short of it. |
...
| `CutoverCompleted` | `True` | `DrainVerified` | The verify Job proved the drain: the target's origin progress sits exactly on the cutover LSN, or `pgcopydb compare data` found every migrated table matching (any distance short of the LSN is decided by content, because publication-filtered WAL and unapplied commits measure alike). Changes applied, sequences synced. |
