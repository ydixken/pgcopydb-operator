...
- Failures the retry cannot fix (wrong clone privileges, a source that dies mid-copy) still burn the budget one attempt at a time, except for what the [preflight](live-migration.md#preflight) catches before the first attempt: connectivity and credentials on every migration, the follow prerequisites on live ones. The `Failed` condition names the pgcopydb error, so check it after the first failed attempt instead of waiting for the budget to drain; classifying the remaining deterministic failures to stop early is an open task.
