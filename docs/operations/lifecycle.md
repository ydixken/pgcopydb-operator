...
- Failures the retry cannot fix (wrong privileges, unreachable host) still burn the budget one attempt at a time, except for the follow prerequisites that [preflight](live-migration.md#preflight) catches before the first attempt. The `Failed` condition names the pgcopydb error, so check it after the first failed attempt instead of waiting for the budget to drain; classifying the remaining deterministic failures to stop early is an open task.
