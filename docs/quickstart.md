...
- `status.progress` (tables, indexes, bytes) comes from `pgcopydb list progress`, which is never run against a live worker, because opening pgcopydb 0.18's catalogs while the copy writes them kills workers: a plain clone like this one gets a single best-effort sample as its worker exits, so the field MAY stay empty here, and a live migration gets its counters from the drain-verification Job after cutover (the database-size metrics are the live view of a running copy). The poll also runs only on runner versions the operator's allowlist names; the bundled runner qualifies. On any other runner, such as a custom stock 0.18 image, the gate stays closed, because that pgcopydb returns no data and corrupts the stored filtering of a filtered work dir (see [Troubleshooting](troubleshooting.md)). `status.conditions` are authoritative; `phase` is a summary for the printer column.
