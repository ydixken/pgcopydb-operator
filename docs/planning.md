...
- [ ] What downtime, RPO, RTO, maintenance window, load impact, and acceptance criteria are agreed?
- [ ] Where will pgcopydb run, can it maintain TLS-verified connections to both endpoints, and which least-privilege roles and credential process will it use?
- [ ] What are the database size, largest relations, write and WAL rates, largest transaction, growth, and available connection, compute, I/O, network, and work-disk capacity?
- [ ] Does the downtime requirement justify follow mode, and has a plain clone rehearsal established copy time and resource use?
- [ ] Is logical decoding ready, including logical WAL, slot and WAL sender capacity, plugin availability, HBA access, retention limits, and slot behavior after failover?
- [ ] Can DDL and automated partition creation stop during follow, or is a target-first DDL process assigned?
- [ ] Do tables receiving updates or deletes have suitable replica identity, and are RLS, triggers, and target-side writers accounted for?
- [ ] Can sequences or large objects change during follow, and how will their final state be synchronized and verified?
...
1. Partial.
   API filter validation and `clone rights schemas` operate on declared scope, but preflight does not confirm customer intent.
2. Not checked.
...
8. Not checked.
   Preflight does not detect planned or concurrent DDL or automated partition creation.
9. Partial.
...
10. Partial outside preflight.
    Follow cutover synchronizes sequences after drain, but preflight does not inventory sequences or large objects or detect large-object changes during follow.
11. Partial outside preflight.
...
14. Not checked.
    Product E2E tests do not prove that a customer rehearsal used representative data or adequate isolation controls.
